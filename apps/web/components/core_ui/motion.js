...
import { Sparkles, MessageCircleHeart, Star, } from "lucide-react";
export function Card_1() {
    return (<motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="relative">
...
export function Card_2() {
    return (<>
        {["Create Rooms", "Meet People", "Chat in Real-time"].map((label, i) => (<motion.div key={label} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.1 }}>
