...
import { Moon, Sun, MessageCircleHeart, Menu, X, } from "lucide-react";
...
function ProfileDropdown() {
    ...
    return (<DropdownMenu>
      <DropdownMenuTrigger asChild>
        <button className="flex items-center justify-center rounded-full sm:rounded-2xl w-10 h-10 sm:w-auto sm:h-auto sm:px-3 sm:py-1.5 sm:gap-2 sm:border sm:border-dashed sm:bg-white/70 sm:dark:bg-neutral-900/70 sm:backdrop-blur-xl hover:shadow-md transition">
          {session.user?.image ? (<img src={session.user.image} alt="profile" className="h-9 w-9 rounded-full"/>) : (<div className="h-9 w-9 rounded-full bg-emerald-400 flex items-center justify-center text-white text-sm font-bold">
              {session.user?.name?.[0]}
            </div>)}
...
export default function Navbar() {
    ...
    return (<header className="fixed top-0 left-0 w-full z-40 bg-transparent">
            <div className="max-w-6xl mx-auto px-4 py-3 flex items-center justify-between">
              ...
              <div className="hidden sm:flex gap-3 items-center p-1.5 bg-white/70 dark:bg-neutral-900/70 backdrop-blur-xl rounded-2xl border border-dashed border-gray-400 dark:border-neutral-700">
                ...
                {/* login/signup dialog */}
               {session ? <ProfileDropdown /> : (<Dialog>
            ...
            {/* mobile menu */}
            <AnimatePresence>
              {open && (<motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} className="sm:hidden bg-white/50 dark:bg-black/40 backdrop-blur-xl">
