...
export default function RoomPage() {
    ...
    return (<div className="relative min-h-screen bg-background">
      ...
      <div className="relative z-10 max-w-[1400px] mx-auto px-2 py-6">
        <div className={cn("relative rounded-3xl bg-card/70 backdrop-blur-xl border shadow-xl", "flex flex-col h-[calc(100vh-8rem)]")}>
          <div className="flex-1 overflow-y-auto px-6 py-6 space-y-3">
            {messages.map((msg) => (<motion.div key={msg.id} initial={{ opacity: 0, y: 8, scale: 0.98 }} animate={{ opacity: 1, y: 0, scale: 1 }} transition={{ duration: 0.18 }} className={cn("flex", msg.mine ? "justify-end" : "justify-start")}>
                <Card className={cn("max-w-[70%] rounded-2xl border shadow-sm", msg.mine
          ...
          {typingUsers.length > 0 && (<div className="px-6 pb-2 text-xs text-muted-foreground">
        ...
        }} onKeyDown={(e) => e.key === "Enter" && sendMessage()} placeholder="Type something calm…"/>
